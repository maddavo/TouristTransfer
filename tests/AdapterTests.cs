...
internal static class AdapterTests
{
    ...
    public static int Main()
    {
        try
        {
            ...
            Contracts.ContractSystem.Instance.Active.Remove(contract);
            Contracts.ContractSystem.Instance.Finished.Add(contract);
            var laterCompleted = new FinePrint.Contracts.TourismContract { Title = "Later completed contract" };
            laterCompleted.Tourists.Add("A");
            Contracts.ContractSystem.Instance.Finished.Add(laterCompleted);
            var latestCompleted = StockContracts.Read(source);
            Check(latestCompleted.Count(g => g.Crew.Contains(a)) == 1 && latestCompleted.Single(g => g.Crew.Contains(a)).Title == "Later completed contract (completed)", "latest completed contract wins duplicate tourist membership");
            Contracts.ContractSystem.Instance.Finished.Remove(contract);
            Contracts.ContractSystem.Instance.Finished.Remove(laterCompleted);
            Contracts.ContractSystem.Instance.Active.Add(contract);
            ...
            contract.Tourists.Add("A");
            var nonStock = new NonStockContract { Title = "Subclass contract" };
            nonStock.Tourists.Add("A");
            Contracts.ContractSystem.Instance.Active.Add(nonStock);
            var subclassGroups = StockContracts.Read(source);
            Check(subclassGroups.Count == 2 && subclassGroups.Single(g => g.Crew.Contains(a)).Title == "Subclass contract", "TourismContract subclasses remain supported and resolve duplicate membership");
            Contracts.ContractSystem.Instance.Active.Remove(nonStock);
            var currentGroups = StockContracts.Read(source);
            var selection = new HashSet<string> { currentGroups.Single(g => g.Crew.Contains(a)).Key(a), currentGroups.Single(g => g.Crew.Contains(b)).Key(b) };
